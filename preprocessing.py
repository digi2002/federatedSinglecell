...
#Count per million (CPM) (30) uses as
#the scaling factor the sum of the read
#counts across all transcripts in a sample
...
def load_largesc(path, dirAdj, dataset, net):
    if dataset == 'Zhengsorted':
        features = pd.read_csv(os.path.join(path + dataset) + '/Filtered_DownSampled_SortedPBMC_data.csv', index_col=0,
                               header=0)
        ...
        features = pd.read_csv(os.path.join(path + dataset) + '/Filtered_Xin_HumanPancreas_data.csv', index_col=0,
                               header=0)

    elif dataset == 'BaronHuman':
        # path = os.path.join(path, 'Pancreatic_data/')
        features = pd.read_csv(os.path.join(path + dataset) + '/Filtered_Baron_HumanPancreas_data.csv', index_col=0,
                               header=0)
        ...
        # path = os.path.join(path, 'Pancreatic_data/')
        features = pd.read_csv(os.path.join(path + dataset) + '/Filtered_Muraro_HumanPancreas_data_renameCols.csv',
                               index_col=0, header=0)

    elif dataset == 'Segerstolpe':
        # path = os.path.join(path, 'Pancreatic_data/')
        features = pd.read_csv(os.path.join(path + dataset) + '/Filtered_Segerstolpe_HumanPancreas_data.csv',
                               index_col=0, header=0)
